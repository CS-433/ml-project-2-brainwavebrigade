...
def plot_voxel_timecourse(predictions, i = 2100, vid = 'BetweenViewings'):
    """
    Plot the real time courses of a voxel (ground truths) and its prediction.
        Parameters:
            predictions (dict): Predictions of the testset, each prediction is stored individually. The keys of the dictionary are the movies titles, and the elements are the model's output arrays each of shape (movie_testset_size, 4330).
            i (int): The voxel to plot. Must be < 4330.
            vid (string): The title of a specific movie.
        Returns:
    """
    
    fMRI = load_masked_fMRI(vid)
    plt.figure(figsize=(12,4))
    plt.plot([i for i in range(fMRI.shape[0])], fMRI[:,i],'lightskyblue', label = 'Ground Truth')
    plt.plot([i for i in range(fMRI.shape[0]) if i%5 == 0], predictions[vid][:,i].numpy(),'g', label='Prediction')
    plt.ylim([-175,175])
    plt.ylabel('Signal amplitude')
    plt.legend(loc='lower right')
    plt.title(f'Evolution of the voxel {i} according to time for the film {vid}')
    plt.xlabel('Time (TR)')
